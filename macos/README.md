...
唯一的活动文件写入是 `auth.json` 和 `config.toml` 中允许的根级路由设置、`cli_auth_credentials_store`。其余配置表和文本保留。API 档继续使用内置 `openai` provider 加 `openai_base_url`，与 Windows 版一致。
...
卸载只需删除应用。共享数据、加密库和钥匙串项不会自动删除；若需清除账号凭据，先确认备份和当前登录。删除不是安全擦除，不保证清除 Time Machine/APFS 快照中的副本。
